...
class DB2_PDO_SQLSRV extends DB2_PDO_SQLServer
{
    /**
    ...
    public function __construct($dsn, $username, $password, $options = array())
    {
        ...
        if (!in_array($this->driver, PDO::getAvailableDrivers())) {
            ...
            try {
                $this->dbh = new PDO("sqlsrv:$dsn", $username, $password);
    ...
    {
        $stmt = parent::_prepare_sql($sql, $options);
    ...
    {
        try {
            $sql = $this->_prepare_sql($sql, $options);
            $stmt = $this->dbh->prepare(trim($sql)); //, array(PDO::ATTR_CURSOR => PDO::CURSOR_SCROLL));
            //print_r($stmt);
            //var_dump($stmt);

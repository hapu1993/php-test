<?php
class Ward_Security_Groups_Permissions extends Object
{
    public $table = "wards";
    public $basic_selection = "t.id, t.name as ward";
    ...
    LEFT JOIN (SELECT g.id as group_id, w.id, p.id as link_id  FROM 
        ward_security_groups_permissions p
    ...
    public $orderby = " hospital, facility, t.name ";
    public $dir = " ASC";
    public $view_array = array(
        'hl7_id' => array('name' => 'HL7 ID','column'=>'hl7_id'),
        'name'=>array("name"=>"Ward",'column'=>'name'),
    ...
    function __construct(){
        ...
        $this->view_array['name'] = array("name"=>$libhtml->local_text['Ward'],'column'=>'name');
    }
    ...
    function _set_table_list_row_items($item){
        ...
        $link = href_link(array(
            ...
            "url"=>$cfg["root"] . "includes/edit_ward_security_group_details.php?subtab=wards&ward_id=".$item->id."&group_id=".$wardSecurityGroups->id.'&link_id='.$item->link_id.'&active_search='.my_request("active_search").'&path='.$libhtml->path,
            "text"=>tick_cross_image(!empty($item->link_id)),
            "tooltip"=>'Activate '.$libhtml->local_text['Ward'].' Permission',
            "popup"=>false,
            "button"=>false,
            "float" => "",
            "clear"=>false));
    ...
    function print_select_ward_group_filter_form($full=true){

...
(function($){
	$.extend({
		tablesorter: new function(){
			...
			this.defaults = {
				...
				dateFormat: "uk",
				onRenderHeader: null,
				selectorHeaders: 'thead th a.sort',
			...
			function buildHeaders(table) {
				...
				.each(function (index) {
					...
					this.column = $(this).parents("th").index();
	...
	ts.addWidget({
		id: 'indexFirstColumn',
		format: function(table) {
			if ( !$(table).hasClass("disable_cn")) {
				for (var i=0; i <= table.tBodies[0].rows.length; i++) $('tbody:first tr:eq(' + (i - 1) + ') td:first', table).html(i);
			}
		}
	});
